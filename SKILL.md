...
- **write_file / read_file commons paths: use LITERAL `<hermes-root>/commons/...`, NEVER a tilde `~/.hermes/commons/...`** — Terminal, `terminal python3`, and `$HOME` all resolve `~` to `/root` (canonical), but `write_file`/`read_file` resolve the `~` TILDE to the Hermes *profile* home (`~/.hermes/profiles/<profile>/home`), NOT `/root`. So a path written with `~/.hermes/commons/...` lands in the phantom tree `~/.hermes/profiles/<profile>/home/.hermes/commons/...` and is never read by anything. **Use the literal absolute prefix `<hermes-root>/commons/...` (no tilde) for finch task-list.json / Action Journal / commons data.** Confirmed 2026-07-18 (finch:scan): journal written with `~` went to `~/.hermes/profiles/<profile>/home/.hermes/commons/journals/ocas-finch/2026-07-18/scan-0705.json`; had to rewrite and `rm` the stray. **`write_file`'s reported `resolved_path` is NOT authoritative for commons paths** (confirmed 2026-07-26 finch:scan): a journal written to the CORRECT literal `<hermes-root>/commons/journals/...` path still reported `resolved_path` under the profile-home tree (`<hermes-home>/commons/...`), but `cp <reported> <expected>` returned "are the same file" — the real inode WAS the canonical `<hermes-root>/commons/...` path. The reported `resolved_path` was misleading, not the actual write. **RULE: after writing commons data, verify the real path with `terminal ls <hermes-root>/commons/...` (or `readlink -f`) — do NOT trust `write_file`'s `resolved_path`, and do NOT over-correct by re-writing into a phantom tree.** A `cp`/"same file" check is the decisive test if you ever doubt where the file landed.
