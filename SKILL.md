...
Full detail (eviction priority, memory_state subcommands) in `references/operational-gotchas.md` § Scripts:
...
- `finch_hooks_plugin.py` — Hermes Agent Hooks plugin for real-time signal capture, memory tool guard, subagent delegation tracking, and prompt section registration. See `references/hermes-hooks-integration.md` for hook patterns.
